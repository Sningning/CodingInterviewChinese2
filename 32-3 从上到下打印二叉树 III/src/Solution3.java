...
public class Solution3 {
     ...
     * 方法二：双栈
     *
     * 输出 1 之后，将 1 的左节点 2 和右节点 3 保存在一个容器中，第二层打印顺序是 3、2，根据输入和输出的顺序可以推断，该容器应该是栈；
     * 第三层打印的时候，还是先打印的 2 的子节点，然后再打印的 3 的子节点，所以存放 2、3 子节点的容器也是栈；
     * 但是注意，第三层打印 2 子节点的时候是先打印的左节点(4)、后打印的右节点(5)，所以压入第二层节点的子节点时，需要先压入右节点，再压入左节点。
     *
     * 总结：用两个栈保存节点；
     * 在遍历奇数层的元素时，每遍历一个，就要向另一个栈中先压入左节点、再压入右节点；
     * 在遍历偶数层的元素时，每遍历一个，就要向另一个栈中先压入右节点、再压入左节点。
     */
